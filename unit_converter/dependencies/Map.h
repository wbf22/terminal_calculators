...
static void insert_no_resize(Map* map, void* key, size_t key_size, void* data, int allow_overwrite) {
...
void m_any_unique(Map* map, void* key, size_t key_size, void* data) {
    ...
    insert_no_resize(map, key, key_size, data, 0);
...
*/
void m_any_put(Map* map, void* key, size_t key_size, void* data) {
...
/*
    Function to insert an object in the map using an int as the key.
*/
void m_int_put(Map* map, int key, void* data) {
    m_any_put(map, &key, sizeof(int), data);
}
...
*/
void m_put(Map* map, char* key, void* data) {
    m_any_put(map, key, (strlen(key) + 1) * sizeof(char), data);
}
